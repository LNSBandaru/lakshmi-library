...
describe('Handler – Unit', () => {
  ...
  function setUp(
    ...
    const pgStub = sinon.stub().callsFake((opts: any) => {
      ...
      if (opts.database === appDatabase) {
        ctorCount++;
        if (ctorCount === 1) return serviceClientStub;
        if (ctorCount === 2 && env.CDC_USER_SECRET) return cdcClientStub;
        return serviceClientStub;
      }
      throw new Error('Unexpected Client instantiation');
    });
    ...
    const consoleSpy = sinon.spy(console, 'log');
    return { handler, pgStub, mainClientStub, serviceClientStub, cdcClientStub, consoleSpy };
  }
  ...
  it('completes happy path (no CDC)', async () => {
    const { handler, mainClientStub } = setUp();
    const result = await handler.handler();
  ...
  it('uses defaults when db/schema undefined', async () => {
    const { handler } = setUp(
      { APP_DATABASE_NAME: undefined, APP_SCHEMA_NAME: undefined },
    );
    const result = await handler.handler();
    expect(result).to.deep.equal({ message: `Database 'myapp' usernames are ready for use!` });
  });

  it('skips create statements when db & users already exist', async () => {
    const { handler, mainClientStub } = setUp(
      {},
      { databaseExists: true, serviceUserExists: true },
    );
  ...
  it('creates CDC user when missing', async () => {
  ...
  it('skips CDC creation if user exists', async () => {
    const { handler, cdcClientStub } = setUp(
      { CDC_USER_SECRET: 'cdc‑test' },
      { cdcUserExists: true },
    );
  ...
  it('skips CDC flow when SecretString undefined', async () => {
    const { handler, pgStub, cdcClientStub } = setUp(
      { CDC_USER_SECRET: 'cdc‑test' },
      {},
      null,
    );
    const result = await handler.handler();
    expect(pgStub.calledThrice).to.be.false;
    expect(cdcClientStub.connect.called).to.be.false;
    expect(result).to.deep.equal({ message: expectedMsg });
  });

  it('skips entire CDC flow when env var absent', async () => {
...
describe('Handler - Unit', () => {
  ...
  it('should create CDC user when missing and apply grants', async () => {
    const { handler, cdcClientStub } = setUp(
      { CDC_USER_SECRET: 'cdc-test' },
      { cdcUserExists: false },
    );
  ...
  it('should skip CDC user creation if user already exists', async () => {
    const { handler, cdcClientStub } = setUp(
      { CDC_USER_SECRET: 'cdc-test' },
      { cdcUserExists: true },
    );
  ...
  it('should skip CDC flow entirely when CDC_USER_SECRET is not set', async () => {
    const { handler, cdcClientStub } = setUp({}, {}, undefined);
    const result = await handler.handler();
    expect(cdcClientStub.connect.called).to.be.false;
    expect(result).to.deep.equal({ message: expectedMsg });
